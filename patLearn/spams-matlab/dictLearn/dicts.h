...
template <typename T> Trainer<T>::Trainer() : _k(0), _initialDict(false),
  _initialWeight(false),_itercount(0), _batchsize(256) { 
      _NUM_THREADS=1;
#ifdef _OPENMP
...
template <typename T> Trainer<T>::Trainer(const int k, const
   ...
   _initialDict(false), _initialWeight(false),_itercount(0),_batchsize(batchsize), 
...
/// Constructor with data: weight included
template <typename T> Trainer<T>::Trainer(const int k, const Matrix<T>& weight,const
      int batchsize, const int NUM_THREADS) : _k(k), 
   _initialDict(false), _initialWeight(true),_itercount(0),_batchsize(batchsize), 
   _NUM_THREADS(NUM_THREADS) { 
	_weight.copy(weight);
      if (_NUM_THREADS == -1) {
         _NUM_THREADS=1;
...
void Trainer<T>::train(const Data<T>& X, const ParamDictLearn<T>& param) {
   ...
   T* const pr_weight=weight.rawX();

   if (_weight.m() != M || _weight.n() != 1) 
      _initialDict=false;
      ...
      for (j = 0; j<batchsize; ++j) {
         ...
         if (param.mode < SPARSITY) {
            if (param.mode == PENALTY && param.lambda==0 && param.lambda2 > 0 && !param.posAlpha) {
               ...
               coreLARS2(DtRj,G,Gs,Ga,invGs,u,coeffs_sparse,ind,work,normX,param.mode,pr_weight[index]*param.lambda,param.posAlpha);
            }
         } else {
            if (param.mode == SPARSITY) {
               coreORMPB(DtRj,G,ind,coeffs_sparse,normX,L,T(0.0),T(0.0));
            } else if (param.mode==L2ERROR2) {
               coreORMPB(DtRj,G,ind,coeffs_sparse,normX,L,pr_weight[index]*param.lambda,T(0.0));
            } else {
               coreORMPB(DtRj,G,ind,coeffs_sparse,normX,L,T(0.0),pr_weight[index]*param.lambda);
            }
         }
         int count2=0;
         for (int k = 0; k<L; ++k) 
