...
template <typename T>
   inline void callFunction(mxArray* plhs[], const mxArray*prhs[],
      ...
      // add weight
      mxArray* pr_weight = mxGetField(prhs[1],0,"weight");
      Trainer<T>* trainer;
	...
	if (!pr_weight){
	  ...
	  T* prweight = reinterpret_cast<T*>(mxGetPr(pr_weight));
	  const mwSize* dimsWeight=mxGetDimensions(pr_weight);
	  int nWeight=static_cast<int>(dimsWeight[0]);
	  int nCol=static_cast<int>(dimsWeight[1]);
	  if (M != nWeight || nCol != 1) mexErrMsgTxt("sizes of weight are not consistent");
	  Matrix<T> weight(prweight,M,1);        
	  trainer = new Trainer<T>(K,weight,batch_size,NUM_THREADS);
	}	  
      } else {
